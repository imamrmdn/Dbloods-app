import 'dart:io';
import 'package:flushbar/flushbar.dart';
import 'package:flutter_spinkit/flutter_spinkit.dart';
import 'package:path/path.dart';

import 'package:dbloods_app/bloc/blocs.dart';
import 'package:dbloods_app/models/models.dart';
import 'package:dbloods_app/shared/data_source.dart';

import 'package:dbloods_app/services/services.dart';
import 'package:dbloods_app/shared/shareds.dart';
import 'package:dbloods_app/ui/widgets/widgets.dart';
import 'package:email_validator/email_validator.dart';
import 'package:firebase_auth/firebase_auth.dart';
import 'package:flutter/material.dart';

import 'package:flutter_svg/flutter_svg.dart';
import 'package:flutter_webview_plugin/flutter_webview_plugin.dart';
import 'package:material_design_icons_flutter/material_design_icons_flutter.dart';
import 'package:pie_chart/pie_chart.dart';
import 'package:provider/provider.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:qr_flutter/qr_flutter.dart';
import 'package:random_string/random_string.dart';
import 'package:smooth_page_indicator/smooth_page_indicator.dart';

part 'on_boarding_screen.dart';
part 'sign_in_screen.dart';
part 'sign_up_screen.dart';
part 'main_screen.dart';
part 'wrapper.dart';
part 'home_screen.dart';
part 'eventdetail_screen.dart';
part 'getdonorid_screen.dart';
part 'succsess_screen.dart';
part 'info_screen.dart';
part 'informasidetail_screen.dart';
part 'edukasidetail_screen.dart';
part 'stok_screen.dart';
part 'profile_screen.dart';
part 'accountConfirmation_screen.dart';
part 'aboutapps_screen.dart';
part 'editprofile_screen.dart';
part 'riwayatdonor_screen.dart';
